...
class MegaDepth1500(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data['image0'] = torch.tensor(
            image0.astype(np.float32)/255).permute(2, 0, 1)
        data['image1'] = torch.tensor(
            image1.astype(np.float32)/255).permute(2, 0, 1)
...
def run_pose_benchmark(matcher_fn, loader, ransac_thr=2.5):
    ...
    for d in tqdm.tqdm(loader):
        d_error = {}
        src_pts, dst_pts = matcher_fn(tensor2bgr(
            d['image0']), tensor2bgr(d['image1']))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run pose benchmark with matcher")
